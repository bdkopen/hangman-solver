	...
	for x in range(0, len(word_input)):
		for match in matches:
			if(word_input[x] == "." and ord(match[x]) >= 97 and ord(match[x]) <= 122):
				...
				letter_total += 1
	return (str(chr(letters.index(max(letters))+97)), str(convertToPercent(max(letters), letter_total)));
...
def getMatches(word_input, letters_guessed):
	# Create an exclusion regex only if letters have been tried.
	regex = word_input
	if(letters_guessed != ""):
		regex = regex.replace(".", "[^"+letters_guessed+"]")
